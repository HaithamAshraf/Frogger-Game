...
void level2(int,int, string);
using namespace std;
using namespace sf;
void level1(string name){
	string Name = name;
	RenderWindow window(VideoMode(1200, 780), "FROGGER!", Style::Close);
	Texture frog, water, stone, road, start, finish, finish2, car1, car2, car3, car4, car5, car6, car7, car8, car9, car10, turtle, root, scorebarbg, heartshape;
	...
	Sprite scorebar_bg, heart, heart2, heart3;
	...
	int i = 0, losecount = 0, score = 0, pixelcountUP = 0, pixelcountDOWN = 0,winner = 0;;
	...
	while (window.isOpen())
	{
		...
		//WARNING: THERE'S WATER GAPS AT THE END, WATCH OUT ;) 
			...
			if (x > 60 && x < 120) {
				...
				score_number = to_string(score);
				scorenumber.setString(score_number);
				frogbg.setPosition(90, 30);
				window.draw(firstround);
				window.draw(pressenter);
				winner = 1;
				if (Keyboard::isKeyPressed(Keyboard::Key::Enter)) {
					window.close(); 
					level2(score,losecount, Name);
				}
			...
			else if (x > 1020 && x < 180) {
				for (i; i < 1; i++)
					score += 50;
				score_number = to_string(score);
				scorenumber.setString(score_number);
				frogbg.setPosition(1050, 30);
				window.draw(firstround);
				window.draw(pressenter);
				winner = 1;
				if (Keyboard::isKeyPressed(Keyboard::Key::Enter)) {
